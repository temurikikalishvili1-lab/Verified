// Checkout page functionality (redirect providers: Bank of Georgia, WooPay)
class CheckoutManager {
    ...
    async loadPaymentProvider() {
        try {
            ...
            this.provider = (data?.provider || this.provider || 'bog').toLowerCase();
            this.currency = (data?.currency || this.currency || 'gel').toLowerCase();
            this.bogAvailable = data?.bogConfigured !== false;
            this.woopayAvailable = data?.woopayConfigured !== false;
            this.providerAvailable = this.isProviderReady();
    ...
    isProviderReady() {
        if (this.provider === 'woopay') return this.woopayAvailable;
        return this.bogAvailable;
    }

    getProviderName() {
        return this.provider === 'woopay' ? 'WooPay Crypto' : 'Bank of Georgia';
    }

    updatePaymentUi() {
        const payBtn = document.getElementById('bog-pay-btn');
        const providerLabel = document.getElementById('payment-provider-label');
        const providerName = this.getProviderName();
        const enabled = this.isProviderReady();

        if (payBtn) {
            payBtn.disabled = !enabled;
            payBtn.textContent = enabled ? `Pay with ${providerName}` : 'Payment unavailable';
        }
    ...
    async handlePayment() {
        ...
        if (!this.isProviderReady()) {
            this.showNotification(`${this.getProviderName()} payments are not configured.`, 'error');
            return;
        }
        ...
        const { items, total } = this.collectOrderItems();
        ...
        if (this.provider === 'woopay') {
            await this.handleWoopayPayment(items, metadata);
            return;
        }

        await this.handleBogPayment(items, metadata);
    }

    async handleBogPayment(items, metadata) {
        this.showNotification('Redirecting to Bank of Georgia...', 'info');

        const order = await this.createBogOrder(items, metadata);
    ...
    async handleWoopayPayment(items, metadata) {
        this.showNotification('Redirecting to WooPay Crypto...', 'info');

        const order = await this.createWoopayOrder(items, metadata);
    ...
    async createWoopayOrder(items, metadata = {}) {
        try {
            const res = await fetch('/api/payments/woopay/create-order', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ items, metadata })
            });
            ...
            console.error('WooPay order creation failed:', err);
            this.showNotification('Network error while starting payment.', 'error');
            return null;
        }
    }
